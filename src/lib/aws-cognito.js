...
const getCurrentUser = () => config.userPool.getCurrentUser();
  ...
  new Promise((resolve, reject) => {
    ...
    switch (provider) {
      ...
      case 'amazon':
        ...
        break;
      default:
        providerKey = `cognito-idp.${AWS.config.region}.amazonaws.com/${config.userPool.userPoolId}`;
        break;
    }
...
export const register = (username, password, email) => (
  new Promise((resolve, reject) => {
    ...
    config.userPool.signUp(username, password, attributeList, null, (err, data) => {
      if (err) {
        reject(err);
      } else {
